...
export class King extends Figure {
    ...
    canMove(target: Cell): boolean {
        ...
        if ((target.x === this.cell.x + direction) && target.y === this.cell.y && this.cell.board.getCell(target.y, target.x).isEmpty()){
            return true;
        }
        if(target.x === this.cell.x + direction && (target.y === this.cell.y + 1 || target.y === this.cell.y - 1) && this.cell.isEnemy(target)){
                return true;
        }
        return false;
    }
